...
export default function CheckoutClientPage({ sessionUser }: Props) {
  ...
  return (
    <main className="min-h-screen px-4 py-8 sm:px-6 lg:px-10" style={{ background: "var(--surface)" }}>
      <div className="mx-auto max-w-7xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_390px]">
          <div className="space-y-5">
            <Card className="rounded-[28px] p-6 shadow-sm sm:p-8">
            ...
            <Card className="rounded-[28px] p-6 shadow-sm sm:p-8">
              ...
              {hasFreeShipping && (
                <div className="mb-4 flex items-center gap-2 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">
                  <Truck size={16} /> Twoje zamówienie kwalifikuje się do darmowej dostawy 🎉
                </div>
              )}
                  ...
                  <label key={option.id} className={`cursor-pointer rounded-2xl border p-4 transition ${selectedShippingMethod === option.id ? "border-emerald-700 bg-emerald-50" : "border-gray-100 bg-white hover:border-gray-300"}`}>
                    ...
                    <span className="flex items-start justify-between gap-4">
                      ...
                      <span className="whitespace-nowrap text-sm font-bold text-gray-950">
                        {option.price === 0 || hasFreeShipping ? (
                          <span className="text-[#3a9a2c]">Gratis</span>
                        ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 grid gap-3">
                    <label className={`cursor-pointer rounded-2xl border p-4 transition ${selectedPaymentMethod !== "POBRANIE" ? "border-emerald-700 bg-emerald-50" : "border-gray-100 bg-white hover:border-gray-300"}`}>
          ...
          <aside className="space-y-4 lg:sticky lg:top-6 lg:h-fit">
            <Card className="rounded-[28px] p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-xl bg-gray-100 px-3 py-2 text-sm font-semibold text-gray-700">{totalItems} szt.</span>
              </div>

              {cart.length === 0 ? (
                <div className="mt-6 rounded-2xl bg-gray-50 p-5 text-gray-600">Koszyk jest pusty.</div>
              ) : (
